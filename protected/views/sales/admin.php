...
<div class="page-size-wrap" style="width:100%">
    <div class="results-perpage">
        ...
        <?php if(Yii::app()->user->partner==$_GET['parent_id'] || Yii::app()->user->userType=="Forkind"){?>
        	<a href="<?php echo $this->createUrl('sales/create',array('id'=>$_GET['parent_id']));?>" style="float:right;padding: 3px 8px;margin-left:10px;" class="btn btn-primary">Add New</a>
        <?php }?>
    </div>
</div>
        ...
        array(
            ...
            'buttons' => array(
                    ...
                    'view' => array( //the name {reply} must be same
                        ...
                        'url' => function($data) {
                            $url = Yii::app()->createUrl('sales/View?id=' . $data->id);
                            return $url;
                         },
                     ),
                  )
...
new DG.OnOffSwitchAuto({
    ...
    listener:function(name, checked){
    	...
    	$.ajax({
        	...
        	url:'<?php echo Yii::app()->createAbsoluteUrl("sales/ChangeStatus")?>',
